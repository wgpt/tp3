...
class Upload{
    ...
    //生成缩略图
    public function img_create_small($big_img, $width, $small_img) {//原始大图地址，缩略图宽度，高度，缩略图地址
        header('Content-Type: text/html;charset=utf-8');
        chmod($big_img, 0777);
        $big_image_size   =   getimagesize($big_img); 
        $radu = $big_image_size[1]/$big_image_size[0];
        $height = $width*$radu;
        //$imgage = getimagesize($big_img); //得到原始大图片
        switch ($big_image_size[2]) { // 图像类型判断
        case 1:
        ...
        $src_W = $big_image_size[0]; //获取大图片宽度
        $src_H = $big_image_size[1]; //获取大图片高度
        imagesavealpha($im,true);//这里很重要
        $tn = imagecreatetruecolor($width, $height); //创建缩略图
        imagealphablending($tn,false);//这里很重要,意思是不合并颜色,直接用$img图像颜色替换,包括透明色;
        imagesavealpha($tn,true);//这里很重要,意思是不要丢了$thumb图像的透明色;
        imagecopyresampled($tn, $im, 0, 0, 0, 0, $width, $height, $src_W, $src_H); //复制图像并改变大小
        imagepng($tn, $small_img); //输出图像
    }
    /**
     * 转换上传文件数组变量为正确的方式
     * @access private
     ...
     * @return array
     */
    public function getUploadFileInfo() {  //xin
